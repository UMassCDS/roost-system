...
parser.add_argument(
    "--station", type=str, required=True, help="a single station name, eg. KDOX"
)
parser.add_argument(
    "--start",
    type=str,
    required=True,
    help="the first local date to process, eg. 20101001",
)
parser.add_argument(
    "--end",
    type=str,
    required=True,
    help="the last local date to process, eg. 20101001",
)
parser.add_argument(
    "--sun_activity",
    type=str,
    default="sunrise",
    help="time window around sunrise or sunset",
)
parser.add_argument(
    "--min_before",
    type=int,
...
parser.add_argument(
    "--just_render",
    action="store_true",
    help="just download and render, no detection and tracking",
)
...
parser.add_argument(
    "--is_canadian", type=bool, default=False, help="to determine data type"
)
...
DIRS = {
    ...
    "vis_det_dir": os.path.join(
        args.data_root, "vis_dets"
    ),  # vis of detections from detection model
    "vis_NMS_MERGE_track_dir": os.path.join(
        args.data_root, "vis_NMS_MERGE_tracks"
    ),  # vis of tracks after NMS & merge
...
days = get_days_list(
    args.start, args.end
)  # timestamps that indicate the beginning of dates, no time zone info
